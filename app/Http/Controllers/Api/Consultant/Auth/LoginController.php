...
{ 
    ...
    */
    ...
    {  
        try {
            ...
            if($token = auth()->guard('consultants')->attempt($credentials)){
                 $consultant = Auth::guard('consultants')->user();
                $consultant->access_token = $token;
              return $this->returnData('consultant',$consultant,trans('response_msg.login_success'));
    ...
    */
    ...
    {
        $value  =   $identify;
        $felid  =   filter_var($value,FILTER_VALIDATE_EMAIL) ? 'email' :'username';
